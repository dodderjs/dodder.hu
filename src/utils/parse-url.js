...
const getAnchorQs = function(location) {
	let anchorQs    = getAnchorSearch(location),
...
const buildCredentials = function(location, keys) {
	let params = getAllParams(location);
	let authHeaders = {};
...
export default function getRedirectInfo(currentLocation) {
	if (!currentLocation) {
		...
		let authRedirectHeaders = buildCredentials(currentLocation, authKeys);
		let authRedirectPath = getLocationWithoutParams(currentLocation, authKeys);
